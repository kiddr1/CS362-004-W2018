//Assignment 5 bug report on Chris M's
...
int playSmithy(struct gameState *state, int *handPos, int *currentPlayer) {
	...
	//BUG INTRODUCED FOR ASSIGNMENT 2: Should be i < 3, but I instead allow the player to draw 4 cards
    for (i = 0; i < 4; i++) {
		drawCard(*currentPlayer, state);
	}
...
int playVillage(struct gameState *state, int *handPos, int *currentPlayer) {
	...
	//discardCard(*handPos, *currentPlayer, state, 0); //BUG INTRODUCED FOR ASSIGNMENT 2: Commented out the discardCard feature
...
int playGreatHall(struct gameState *state, int *handPos, int *currentPlayer) {
	//+1 Card
	drawCard(*currentPlayer, state);
	drawCard(*currentPlayer, state); //BUG INTRODUCED FOR ASSIGNMENT 2: Draw an extra card when playing Great Hall

	//+1 Actions
	state->numActions--; //BUG INTRODUCED FOR ASSIGNMENT 2: Decrease actions by 1 instead of increase
